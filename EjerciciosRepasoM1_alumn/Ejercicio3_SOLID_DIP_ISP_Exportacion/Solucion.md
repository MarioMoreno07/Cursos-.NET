...
2. Explica por qué `ServicioInformes` no depende de una clase concreta. Al tener IExportador 
	como argumento puede acepta cualquier tipo de documento
3. Añade `ExportadorJson`.
4. Comprueba que no es necesario modificar `ServicioInformes`.
	No necesita ser modificado ya que al aceptar una clase que implementa IExplorador puede aceptar ExpotadorJSON sin necesidad de modificacion
